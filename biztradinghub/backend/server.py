...
_persisted_rounds: list = round_history.load_all()  # oldest first, loaded once at import time
...
def _seed_history(minutes: int = 30):
    try:
...
def _live_loop():
    ...
    _seed_history()
    _refresh_candles()
    _last_closed_bar_len = len(engine.bar_builder.bars)  # don't treat the whole preload as "just closed"
    _persist_new_rounds()  # in case any window closed during the 30-min preload
    while True:
        try:
            price = feed.refresh()
...
def get_candles():
    with _state_lock:
        return list(_recent_candles[-1000:])
